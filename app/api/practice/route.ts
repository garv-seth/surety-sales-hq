...
export async function PUT(request: Request) {
  // Generate final report for a practice session
  try {
    const body = await request.json();
    const { conversationHistory, scores, businessType, scenario } = body;
...
Average score: ${(scores.reduce((a: number, b: number) => a + b, 0) / scores.length).toFixed(1)}/10

Based on a typical practice session of this type, provide a coaching report as JSON:
{
  "finalScore": ${(scores.reduce((a: number, b: number) => a + b, 0) / scores.length).toFixed(1)},
